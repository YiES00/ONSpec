...
- A등급(실측 연계)은 Tyto Robotics DB(S3) 어댑터로 발행된다 — KV·무게 등 측정 속성과,
  **조건 매칭을 통과한 실측 최대 추력**(프로펠러 직경 ±5%·피치 ±10%·전압 ±12% 일치 +
  풀스로틀 도달 테스트만). 예: MN4006 KV380 실측 1,546g(제조사 주장 대비 편차 5.6% —
  "실측 확인"으로 병기). 조건이 다른 실측은 동일 지표가 아니므로 채택하지 않는다
  (U15Ⅱ의 유일 테스트는 풀스로틀 미도달로 기각). 숨기지 않는다.
